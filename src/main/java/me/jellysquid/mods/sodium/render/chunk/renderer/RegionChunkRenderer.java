...
public class RegionChunkRenderer extends ShaderChunkRenderer {
    ...
    private void setModelMatrixUniforms(ChunkShaderInterface shader, ChunkRenderMatrices matrices, RenderRegion region, ChunkCameraContext camera) {
        ...
        Matrix4f modelViewProjection = null;
        try {
        modelViewProjection = (Matrix4f) matrices.projection().clone();
        modelViewProjection.mul(matrix);
        matrix.translate(x, y, z);
        modelViewProjection.translate(x, y, z);
        Matrix4f normal = (Matrix4f) matrix.clone();
        normal.invert();
        normal.transpose();
        shader.setModelViewMatrix(matrix);
        shader.setModelViewProjectionMatrix(modelViewProjection);
